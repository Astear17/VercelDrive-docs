...
## Biến môi trường và tài liệu liên quan

Các biến này thay thế cách cấu hình cũ trong `config/api.config.js` và `config/site.config.js`.

| Biến | Bắt buộc | Tài liệu liên quan |
| --- | --- | --- |
| `NEXT_PUBLIC_SITE_TITLE` | Có | Xem [Tùy chỉnh cấu hình](./custom-configs#site-title). |
| `USER_PRINCIPAL_NAME` | Có | Email tài khoản OneDrive. Xem [Bắt đầu](./getting-started#3-chuan-bi-bien-moi-truong). |
| `BASE_DIRECTORY` | Có | Xem [Tùy chỉnh cấu hình](./custom-configs#base-directory). |
| `CLIENT_ID` | Có | Lấy từ trang overview của Microsoft Entra App Registration. |
| `CLIENT_SECRET` | Có | AES-obfuscate bằng công cụ ở trên. |
| `REDIS_URL` | Có | Xem [Cache](./cache) về Redis và lưu token. |
| `UPLOAD_PASSWORD` | Chỉ khi bật upload | Xem [Cổng mật khẩu tải lên](#cong-mat-khau-tai-len). |
| `NEXT_PUBLIC_PROTECTED_ROUTES` | Tùy chọn | Xem [Thư mục bảo vệ bằng mật khẩu](./features/protected-folders). |
| `KV_PREFIX` | Tùy chọn | Dùng khi nhiều bản triển khai dùng chung một Redis database. |
| `NEXT_PUBLIC_EMAIL` | Tùy chọn | Xem [Tùy chỉnh cấu hình](./custom-configs#contact-email). |
| `UPLOAD_CONFLICT_BEHAVIOR` | Tùy chọn | Xem [Xử lý trùng tên khi tải lên](#xu-ly-trung-ten-khi-tai-len). |
